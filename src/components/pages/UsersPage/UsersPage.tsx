...
import { clearGetUsers, deleteUser, getAllUsers } from '@/redux/reducers/usersList/actions';
...
const UsersPage = () => {
  ...
  const [updateModal, setUpdateModal] = useState<Partial<UserInterface> | null>(null)
  ...
  useEffect(() => {
    if(successGetUsers && usersList) {
      const mappedData = usersMapper(usersList, handleUpdateModal, handleDeleteModal)
  ...
  return (
    <section className={styles.pageContainer}>
      <Typography sx={{ marginBottom: '20px', color: '#fff'}} variant="h3">Administracion de usuarios</Typography>
      <div className={styles.tounamentsContainer}>
        <div className={styles.listHeader}>
          <Typography variant="h5">Lista de Usuarios</Typography>
          <span className={styles.UsersPageRegisterUser}>
            <Button onClick={handleOpenModal} variant="contained" endIcon={<Person />}>Nuevo Usuario</Button>
          </span>
        </div>
        <div className={styles.listBody}>
          {loadingGetUsers 
            ? <div className={styles.loading}>
                ...
                <Typography variant="h6" style={{ marginTop: '20px' }}>Cargando usuarios...</Typography>
              </div>
            : allUsers.length > 0
              ? <ReactTable 
              ...
              : (
                <div className={styles.loading}>
                  ...
                  <Typography variant="h6" style={{ marginTop: '20px' }}>No hay usuarios registrados...</Typography>
                </div>
              )
          }
        </div>
